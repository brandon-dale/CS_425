...
void julia(Complex d, Complex center, Color* pixels) {
    ...
    float real_off = -0.743643887037151;
    float imag_off = 0.131825904205330;
    Complex c(x*d.x+real_off, y*d.y+imag_off);
    c -= center;
    Complex z(0.0, 0.0);
...
int main() {
    float zoomFactor = 210350;
    float val = 2.1 - 0.0;
    Complex ll(-val / zoomFactor, -val / zoomFactor);
    Complex ur( val / zoomFactor,  val / zoomFactor);
